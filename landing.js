...
.btn{display:inline-flex;align-items:center;padding:13px 22px;border-radius:14px;font-weight:800;font-size:.95rem;text-decoration:none;border:none;transition:transform .2s,box-shadow .2s}
.btn:hover{transform:translateY(-2px)}
.bp{background:linear-gradient(135deg,#e8a04a,#d4783a);color:#1a0f05;box-shadow:0 8px 28px rgba(232,160,74,.35)}
.bgh{background:var(--g);backdrop-filter:blur(12px);color:var(--t);border:1px solid var(--gb)}
...
.p{display:flex;align-items:center;justify-content:center;padding:16px 12px;text-align:center;text-decoration:none;color:var(--t);font-weight:700;font-size:.9rem;border-radius:16px;transition:transform .2s}
.p:hover{transform:translateY(-3px);border-color:rgba(232,160,74,.35)}
.pl{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px}
.pl .c{display:flex;gap:12px;align-items:flex-start;padding:16px;border-radius:16px;transition:transform .2s}
.pl .c:hover{transform:translateY(-2px)}
.pl .ico{font-size:1.4rem;width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.1);border-radius:10px;flex-shrink:0}
.pl b{font-size:.92rem;display:block}
.pl span{color:var(--m);font-size:.8rem}
footer{margin-top:12px;padding:28px 5vw 40px;border-top:1px solid rgba(255,255,255,.06);text-align:center;color:var(--m);font-size:.85rem}
footer a{color:var(--a);text-decoration:none;font-weight:600;margin:0 8px}
...
<a class="btn bgh" href="${m}" target="_blank" rel="noopener">manifest.json</a>
</div>
<div class="box glass">
<label class="lang-fa">لینک منیفست — کپی و در استریمیو نصب کنید</label>
<label class="lang-en">Manifest URL — copy &amp; install in Stremio</label>
...
<p class="sub lang-fa">در کنار سینماگرافی این‌ها را هم امتحان کنید.</p>
<p class="sub lang-en">Great companions alongside Cinemagraphy.</p>
<div class="pl">
<div class="c glass"><span class="ico">🎬</span><div><b>Torrentio</b><span>Torrent / Debrid</span></div></div>
<div class="c glass"><span class="ico">📡</span><div><b>MediaFusion</b><span>Multi-source</span></div></div>
<div class="c glass"><span class="ico">🗂️</span><div><b>101Catalogs</b><span>Catalogs</span></div></div>
<div class="c glass"><span class="ico">🌸</span><div><b>Anime Catalogs</b><span>Anime</span></div></div>
<div class="c glass"><span class="ico">💬</span><div><b>SubSource</b><span>Subtitles</span></div></div>
<div class="c glass"><span class="ico">📺</span><div><b>IPTV Bridge</b><span>Live TV</span></div></div>
</div>
</section>
</main>
<footer>
<a href="${PUBLIC_SITE}">cinemagraphy.vercel.app</a>
<a href="${m}">manifest.json</a>
<a href="https://github.com/TheNerdCow/CinemaGraphy" target="_blank" rel="noopener">GitHub</a>
<p style="margin-top:10px" class="lang-fa">سینماگرافی v${ver}</p>
<p style="margin-top:10px" class="lang-en">Cinemagraphy v${ver}</p>
</footer>
<script>
(function(){
...
if(cb&&inp)cb.onclick=async()=>{try{await navigator.clipboard.writeText(inp.value);cb.classList.add('ok');cb.textContent=r.lang==='fa'?'کپی شد':'Copied';setTimeout(()=>{cb.classList.remove('ok');cb.textContent=r.lang==='fa'?'کپی':'Copy'},1800)}catch{inp.select()}};
})();
</script>
</body></html>`
}
