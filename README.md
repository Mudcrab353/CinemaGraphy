...
> **درباره‌ی Anime Catalogs:** آیدی این کاتالوگ‌ها `kitsu:` هست، نه IMDb — یعنی فعلاً فقط مرور و پوستر/توضیحات کار می‌کنه، پخش خودکار وصل نیست. برای وصل‌شدن پخش، باید یه لایه‌ی تطبیق اسم اضافه بشه (جستجوی اسم انیمه تو پروایدرهای خودمون) که هنوز پیاده نشده.
