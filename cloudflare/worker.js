...
async function catalogResponse(route, providers, logger, env = {}, httpClient) {
    try {
        ...
        if (externalSource) return json(await proxyExternalCatalog(externalSource, route.type, route.id, route.extraArgs, httpClient, logger))
...
async function metaResponse(route, providers, services, env, requestUrl, logger, httpClient) {
    try {
        ...
        if (route.id.startsWith('tt') && env.TMDB_API_KEY) {
                ...
                if (route.type === 'series') {
                    try {
                        ...
                        if (Array.isArray(cin?.meta?.videos) && cin.meta.videos.length) {
                            ...
                            return json({meta: await enrichMetaWithFaTmdb(tmdbMeta, route.type, httpClient, env.TMDB_API_KEY, logger, route.id) || tmdbMeta})
                        }
                    } catch {}
                }
            ...
            try {
                const cin = await services.getCinemeta(route.type, route.id)
                if (cin?.meta) return json({meta: await enrichMetaWithFaTmdb(cin.meta, route.type, httpClient, env.TMDB_API_KEY, logger, route.id) || cin.meta})
            } catch {}
            return json({})
        }
...
async function streamsByTitle(title, type, season, episode, providers) {
    const cleanTitle = title.replace(/[\u0600-\u06FF]/g, '').trim().toLowerCase()
    const settled = await Promise.allSettled(providers.map(async (provider) => {
        const results = await provider.search(title)
        const match = results.find((r) => {
            const cleanName = r.name.replace(/[\u0600-\u06FF]/g, '').toLowerCase()
        ...
        return {streams: (Array.isArray(links) ? links : []).map((link) => ({
            url: link.url,
            title: formatStreamTitle({providerKey: provider.key, quality: link.quality, size: link.size, audioType: link.audioType, extraText: link.title, url: link.url}),
        }))}
    }))
    return sortByQuality(settled.filter((r) => r.status === 'fulfilled').flatMap((r) => r.value.streams))
}
...
async function imdbStreamResponse(type, id, providers, services, env, httpClient, logger) {
    ...
    const streams = title ? await streamsByTitle(title, type, parsed.season, parsed.episode, providers) : []
...
async function kitsuStreamResponse(type, id, providers, env, httpClient, logger) {
    ...
    const streams = title ? await streamsByTitle(title, 'series', parsed.episode ? 1 : null, parsed.episode, providers) : []
    return json({streams: [...streams, ...await torrentPromise]})
}
            ...
            if (url.pathname === '/' || url.pathname === '') {
                ...
                try {
                    ...
                    for (const source of externalSources) {
                        manifest.catalogs.push(...source.catalogs.map((catalog) => ({...catalog, name: translateCatalogName(catalog.name, catalog.type)})))
                        if (source.hasMeta) {
