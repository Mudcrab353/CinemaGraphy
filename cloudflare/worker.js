...
async function handleTmdbImageProxy(request, size, filePath, fetcher = fetch) {
    ...
    if (!parsed) {
        return new Response('invalid tmdb image path', {status: 400, headers: {'content-type': 'text/plain'}})
    }
    try {
        const upstream = await fetcher(parsed.upstream, {
        ...
        if (!ct.startsWith('image/') && !ct.includes('octet-stream')) {
            return new Response('upstream not image', {status: 502, headers: {'content-type': 'text/plain'}})
        }
        const buf = await upstream.arrayBuffer()
        if (buf.byteLength > 8 * 1024 * 1024) {
            return new Response('image too large', {status: 502, headers: {'content-type': 'text/plain'}})
        }
    ...
    return async function workerFetch(request, env = {}) {
        ...
        try {
            ...
            if (url.pathname === '/' || url.pathname === '') {
                ...
                const size = slash >= 0 ? rest.slice(0, slash) : rest
                const file = slash >= 0 ? rest.slice(slash + 1) : ''
                response = await handleTmdbImageProxy(request, size, file, options.fetcher ?? fetch)
